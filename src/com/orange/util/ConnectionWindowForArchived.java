...
public class ConnectionWindowForArchived extends CustomJFrame {
	...
	*/
	...
	CustomJCheckBox validateFromDataBaseForArchived;
	Properties properties = CommonUtils.getArchivalDBProperties();
	
	public ConnectionWindowForArchived() {
		setLayout(null);
	...
	class ExitAction implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				ConnectionBeanArchived.setDbName(conList.getSelectedItem().toString());
			...
			try {
				if (null == ConnectionForArchived.archivedconnection) {
					...
					ConnectionForArchived.archivedconnection = DriverManager.getConnection(url, ConnectionBeanArchived.getUserName(),ConnectionBeanArchived.getPassword());

				}
				setVisible(false);

			} catch (Exception e1) {
				if(null != validateFromDataBaseForArchived){
					validateFromDataBaseForArchived.setSelected(false);
				}
	...
	public void setCheckBox(CustomJCheckBox validateFromDataBaseForArchived) {
		this.validateFromDataBaseForArchived = validateFromDataBaseForArchived;
	}
